...
#[cfg(target_arch = "wasm32")]
pub fn con_log(s: &str) {
    log(s);
}

pub type FileSourceBook = HashMap<String, Vec<u8>>;
...
pub struct ShaderState {
    ...
    pub file_source_book: FileSourceBook,
}
...
impl ShaderState {
        ...
        ) -> Result<(Vec<Instance>, Vec<Light>, Vec<Instance>)>
    {
        ...
        let ad_info = adapter.get_info();
        #[cfg(not(target_arch = "wasm32"))]
        println!("{} {:?}", ad_info.name, ad_info.backend);
        #[cfg(target_arch = "wasm32")]
        con_log(format!("{} {:?}", ad_info.name, ad_info.backend).as_str());
    ...
    pub fn update<F>(&mut self, dt_sec: f32, f: F) -> Result<()>
    where
        F: Fn(&mut Self) -> Result<()>
    {
        self.camera_setting.camera_controller
            .update_camera(&mut self.camera_setting.camera, dt_sec);
...
fn create_render_pipeline(
    ...
    sc_desc: &wgpu::SwapChainDescriptor,
    vert_src: wgpu::ShaderModuleDescriptor,
    frag_src: wgpu::ShaderModuleDescriptor,
