use crate::shader_settings::{texture, FileSourceBook};
use anyhow::*;
use std::path::*;
use std::ops::Range;
...
impl Model {
    pub fn load<P: AsRef<Path>>(
        ...
        let (obj_models, obj_materials) = {
            ...
            {
                tobj::load_obj(path.as_ref(), true)?
            }

            #[cfg(target_arch = "wasm32")]
            {
                let p = (path.as_ref().to_string_lossy()).to_string();
                let reader = file_source_book.get(&p)
                    .with_context(|| {
                        format!("file_source_book: File Not Found! (obj: {})", p)
                    })?;
                let mut reader = reader.as_slice();

                // avoid OsStr
                let path_vec = p.split("/").collect::<Vec<_>>();
                let parent = path_vec[..path_vec.len()-1].join("/");
                tobj::load_obj_buf(&mut reader, true, |mtl_path| {
                    let mp = mtl_path.to_string_lossy();
                    let p = format!("{}/{}", parent, mp);
                    // sorry, but I don't come up with how to return Result val about mtl.
                    let mtl_reader = file_source_book.get(&p)
                        .expect("file_source_book: File Not Found! (mtl)");
                    let mut mtl_reader = mtl_reader.as_slice();
        ...
        for mat in obj_materials {
            ...
            let matuni_buffer_size = std::mem::size_of::<MaterialUniform>() as wgpu::BufferAddress;
            let diffuse_texture = if let Some(t) = diffuse_texture_w {
                ...
                texture::Texture::load(
                    ...
                    file_source_book,
                ).context("default_texture.png を用意してください！")?
            };
                    ...
                    entries: &[
                        ...
                        wgpu::BindGroupEntry {
                            ...
                            resource: wgpu::BindingResource::Buffer {
                                buffer: &matuni_buffer,
                                offset: 0,
                                size: wgpu::BufferSize::new(matuni_buffer_size),
                            }
                        },
                    ],
                    label: None,
...
impl ModelInstanceGroupBook {
    pub fn new(
        ...
        let group_book = instance_sort.into_iter().map(|(model, sorted)| {
            ...
            let buffer_size = (initial_data.len() * std::mem::size_of::<InstanceRaw>() as usize)
                as wgpu::BufferAddress;

            // ! You have to change bind_group when you add/sub Instance.
            let bind_group = device.create_bind_group(
                &wgpu::BindGroupDescriptor {
                    layout,
                    entries: &[
                        wgpu::BindGroupEntry {
                            ...
                            resource: wgpu::BindingResource::Buffer {
                                buffer: &buffer,
                                offset: 0,
                                size: wgpu::BufferSize::new(buffer_size),
                            }
                        }
                    ],
